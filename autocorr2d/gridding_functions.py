# define gridding functions to sample unevenly spaced data onto
# a regular grid
...
# Evaluate Kaiser window function and its Fourier transform
...
# Evaluate spheroidal functions and their Fourier transforms
#
# Ported from AIPS 31DEC15, SPHFN.FOR
# 
# Retrieved from
# https://raw.githubusercontent.com/SKA-ScienceDataProcessor/crocodile/master/oskar_data/scripts/sphfn.py
# on 2016 March 22
# 
# Some tests added by RLW
...
def sphfn(eta, exponent_id, support, gridding=True, normalize=False, **kw):
    ...
    Args:
        ...
        gridding (bool): Flag determines if the spheroidal function or its FT
                      ...
                      If False, generate its FT.
                      (default=True)
        normalize (bool): If True, normalize the function so its integral is
                      unity instead of having peak unity.  Default is False.
                      This only applies if gridding is also True.
    Returns (float):
    ...
    w = np.where(np.abs(eta) <= 1)
    etaw = eta[w]
    eta2 = etaw**2
    j = exponent_id
    if support == 4:
        sphnorm = [ 1.1825277, 1.0226464, 0.87458240, 0.74636706, 0.63790190 ] # support=4
        p4 = [
        ...
        sphnorm = [ 2.2360642, 2.1578855, 2.0830198, 2.0117918, 1.9443885 ] # support=5
        p5 = [
        ...
        p5 = np.array(p5, dtype='f8', order='F').reshape((7, 5), order='F')
        q5 = [
            2.418820e-1, 2.291233e-1, 2.177793e-1, 2.075784e-1, 1.983358e-1]
        q5 = np.array(q5, dtype='f8')
        x = eta2 - 1.0
        psi = (p5[0, j] + x * (p5[1, j] + x * (p5[2, j] + x * (p5[3, j] +\
            x * (p5[4, j] + x * (p5[5, j] + x * p5[6, j])))))) / (1.0 +
            x * q5[j])
    elif support == 6:
        sphnorm = [ 2.4494895, 2.3794136, 2.3116550, 2.2464129, 2.1838293 ] # support=6
        p6l = [
        ...
        p6u = np.array(p6u, dtype='f8', order='F').reshape((5, 5), order='F')
        q6u = [
            1.101270e+0, 3.858544e-1, 1.025431e+0, 3.337648e-1, 9.599102e-1,
            2.918724e-1, 9.025276e-1, 2.575336e-1, 8.517470e-1, 2.289667e-1]
        q6u = np.array(q6u, dtype='f8', order='F').reshape((2, 5), order='F')
        psi = np.zeros(etaw.shape, dtype=float)
        w1 = np.where(np.abs(etaw) > 0.75)
        x = eta2[w1] - 1.0
        psi[w1] = (p6u[0, j] + x * (p6u[1, j] + x * (p6u[2, j] +
            x * (p6u[3, j] + x * p6u[4, j])))) / (1.0 + x * (q6u[0 ,j] +
            x * q6u[1, j]))
        w1 = np.where(np.abs(etaw) <= 0.75)
        x = eta2[w1] - 0.5625
        psi[w1] = (p6l[0, j] + x * (p6l[1, j] + x * (p6l[2, j] +
            x * (p6l[3, j] + x * p6l[4, j])))) / (1.0 + x * (q6l[0, j] +
            x * q6l[1, j]))
    elif support == 7:
        sphnorm = [ 2.6457515, 2.5816600, 2.5193169, 2.4588464, 2.4003518 ] # support=7
        p7l = [
        ...
        p7u = np.array(p7u, dtype='f8', order='F').reshape((5, 5), order='F')
        q7u = [
            1.450730e+0, 6.578685e-1, 1.353872e+0, 5.724332e-1, 1.269924e+0,
            5.032139e-1, 1.196177e+0, 4.460948e-1, 1.130719e+0, 3.982785e-1]
        q7u = np.array(q7u, dtype='f8', order='F').reshape((2, 5), order='F')
        psi = np.zeros(etaw.shape, dtype=float)
        w1 = np.where(np.abs(etaw) > 0.775)
        x = eta2[w1] - 1.0
        psi[w1] = (p7u[0, j] + x * (p7u[1, j] + x * (p7u[2, j] +
            x * (p7u[3, j] + x * p7u[4, j])))) / (1.0 + x * (q7u[0, j] +
            x * q7u[1, j]))
        w1 = np.where(np.abs(etaw) <= 0.775)
        x = eta2[w1] - 0.600625
        psi[w1] = (p7l[0, j] + x * (p7l[1, j] + x * (p7l[2, j] +
            x * (p7l[3, j] + x * p7l[4, j])))) / (1.0 + x * (q7l[0, j] +
            x * q7l[1, j]))
    elif support == 8:
        sphnorm = [ 2.8284281, 2.7690003, 2.7109511, 2.6543690, 2.5993270 ] # support=8
        p8l = [
        ...
        p8u = np.array(p8u, dtype='f8', order='F').reshape((6, 5), order='F')
        q8u = [
            1.379457e+0, 5.786953e-1, 1.300303e+0, 5.135748e-1, 1.230436e+0,
            4.593779e-1, 1.168075e+0, 4.135871e-1, 1.111893e+0, 3.744076e-1]
        q8u = np.array(q8u, dtype='f8', order='F').reshape((2, 5), order='F')
        psi = np.zeros(etaw.shape, dtype=float)
        w1 = np.where(np.abs(etaw) > 0.775)
        x = eta2[w1] - 1.0
        psi[w1] = (p8u[0, j] + x * (p8u[1, j] + x * (p8u[2, j] +
            x * (p8u[3, j] + x * (p8u[4, j] + x * p8u[5, j]))))) / (1.0 +
            x * (q8u[0, j] + x * q8u[1, j]))
        w1 = np.where(np.abs(etaw) <= 0.775)
        x = eta2[w1] - 0.600625
        psi[w1] = (p8l[0, j] + x * (p8l[1, j] + x * (p8l[2, j] +
            x * (p8l[3, j] + x * (p8l[4, j] + x * p8l[5, j]))))) / (1.0 +
            x * (q8l[0, j] + x * q8l[1, j]))
    else:
        raise ValueError('invalid support width')

    if gridding and normalize:
        psi = psi/sphnorm[j]

    rv = np.zeros(eta.shape, dtype=float)
    if not gridding or exponent_id == 0:
        rv[w] = psi
    else:
        rv[w] = (1.0 - eta2)**alpha[exponent_id] * psi
    if scalar:
        return rv[0]
    else:
        return rv
...
if __name__ == "__main__":
    ...
    print("test fdft_method_default")
    ...
    # print table of info from various functions
    xgrid = np.arange(6,dtype=float)/5.0

    print("sphfn vs gridding, exponent_id, support")
    print(f"x    " +
          "".join([f"{x:13.3f}" for x in xgrid]))
    for gridding in [False,True]:
        for exponent_id in [0,2,4]:
            for support in range(4,9):
                f = sphfn(xgrid, exponent_id, support, gridding=gridding)
                print(f"{int(gridding)} {exponent_id} {support}" +
                      "".join([f"{x:13.8f}" for x in f]))

    print("kaiser vs gridding, support")
    print("x  " +
          "".join([f"{x:13.3f}" for x in xgrid]))
    for gridding in [False,True]:
        for support in range(4,9):
    ...
    print("binfunc vs gridding, order")
    print("x  " +
          "".join([f"{x:13.3f}" for x in xgrid]))
    for gridding in [False,True]:
